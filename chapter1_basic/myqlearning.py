...
class QlearningAgent():
    ...
    def learn(self,state,action,reward,next_state,done):    #反向传播
        cur_Q = self.Q[state , action]
        target_Q = reward + ( 1 - float(done) ) * self.gamma * self.Q[next_state, :].max()   #SARSA算法公式中目标值部分
...
def train_episode(env,agent,is_render):    #需要反向传播
    ...
    while True:
        ...
        next_state,reward,done,info = env.step(action) #与环境交互并生成一系列值

        agent.learn(state,action,reward,next_state,done)    #训练起来
